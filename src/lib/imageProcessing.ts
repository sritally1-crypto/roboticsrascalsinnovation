...
export const analyzeImageQuality = (canvas: HTMLCanvasElement): ImageQualityMetrics => {
  ...
  // Calculate brightness and basic metrics
  ...
  const normalizedSharpness = Math.min(sharpness / (canvas.width * canvas.height * 100), 1);
  
  // Quality thresholds
  const isBlurry = normalizedSharpness < 0.1;
  const isOverexposed = normalizedBrightness > 0.9;
  const isUnderexposed = normalizedBrightness < 0.1;
  
  // Overall quality score
  let score = 1;
  if (isBlurry) score -= 0.4;
  if (isOverexposed || isUnderexposed) score -= 0.3;
  if (normalizedBrightness < 0.3 || normalizedBrightness > 0.8) score -= 0.1;
...
// Enhanced background removal for artifact isolation
...
// Enhanced image preprocessing for better 3D reconstruction
...
// Camera constraints for high-quality artifact capture
